...
function Export({
  ...
  return (
    <button
      className="bg-amber-200 text-amber-700 mr-2 px-4 text-base uppercase py-1 my-2 font-semibold rounded-md"
      disabled={disabled}
...
// main component
export default function HistoriData() {
  ...
  async function getData() {
    try {
      ...
      const res = await fetch("https://api-wtf.000webhostapp.com/getDht.php");
  ...
  useEffect(() => {
    ...
    document.title = "Histori Data - Suhu kompor";
  ...
  return (
    <div className="p-5 md:px-5 px-0">
      <h1 className="my-5 text-2xl font-bold text-amber-700">
        {loading && !data ? "Loading Data..." : "Halaman Histori Data"}
      </h1>
      ...
      <DataTable
        title={
          <h4 className="mt-3 font-semibold text-xl text-amber-600">
            Histori Data Suhu
          </h4>
        }
        columns={[
